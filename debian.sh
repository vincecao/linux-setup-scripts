...
BREW_PATH_COMMAND="$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)"
BREW_DIR="/home/linuxbrew"
if [ -d "$BREW_DIR" ]; then
  echo "Do you want to reinstall brew? (yes), followed by [ENTER]"
  select yn in "Yes" "No"; do
    case $yn in
        Yes ) echo "Reinstalling brew start"; sudo rm -rf $BREW_DIR; yes | /bin/bash -c "$BREW_INSTALL_COMMAND"; echo "eval \"$BREW_PATH_COMMAND\"" >> $HOME/.profile; eval "$BREW_PATH_COMMAND"; break;;
        No ) echo "Installing brew abort"; break;;
    esac
  done
else
  echo "Installing brew start"
  yes | /bin/bash -c "$BREW_INSTALL_COMMAND" && echo "eval \"$BREW_PATH_COMMAND\"" >> $HOME/.profile && eval "$BREW_PATH_COMMAND"
fi
